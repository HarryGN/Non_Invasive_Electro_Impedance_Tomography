...
if ischar(list) && strcmp('ERROR',list(1:5))  %% strcmp 两个字符串相同返回1
    disp('Error1: EnumSerialComs - No SERIALCOMM registry entry')
    return;
end
...
out = 0;
outK = 0;
for j=1:1%is use ftdi usb2ttl shoulid change this
    switch j
        case 1
...
%             key = 'HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Enum\FTDIBUS\';
    ...
    if ischar(vals) && strcmp('ERROR',vals(1:5))
        disp('Error2: EnumSerialComs - No Enumerated USB registry entry')
        return;
    end
